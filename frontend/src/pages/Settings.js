...
import { Bell, User, Trash2, RefreshCw, Globe, Users, AlertCircle, Shield } from 'lucide-react';
...
export default function Settings() {
  ...
  const [selectedUrgeType, setSelectedUrgeType] = useState(user?.urge_type || '');
  ...
  const saveUrgeType = async () => {
    try {
      const res = await axios.put(`${API}/profile`, { urge_type: selectedUrgeType, custom_urge_type: selectedUrgeType === 'other' ? customUrge : null }, { withCredentials: true });
      setUser(prev => ({ ...prev, ...res.data }));
    } catch (error) { console.error('Failed to save urge type:', error); }
  };

  const toggleDay = (day) => setReminders(prev => ({ ...prev, days: prev.days.includes(day) ? prev.days.filter(d => d !== day) : [...prev.days, day] }));
  ...
  const addBuddy = async () => {
    ...
    try {
      const res = await axios.post(`${API}/buddies`, { name: buddyName, email: buddyEmail || null, phone: buddyPhone || null, relationship: buddyRelation || null }, { withCredentials: true });
      setBuddies([...buddies, res.data]);
      setBuddyDialogOpen(false); setBuddyName(''); setBuddyEmail(''); setBuddyPhone(''); setBuddyRelation('');
    } catch (error) { console.error('Failed to add buddy:', error); }
  };

  const alertBuddy = async (buddyId) => {
    try {
      const res = await axios.post(`${API}/buddies/${buddyId}/alert`, {}, { withCredentials: true });
      alert(res.data.message);
    } catch (error) { console.error('Failed to alert buddy:', error); }
  };

  const deleteBuddy = async (buddyId) => {
    try {
      await axios.delete(`${API}/buddies/${buddyId}`, { withCredentials: true });
      setBuddies(buddies.filter(b => b.buddy_id !== buddyId));
    } catch (error) { console.error('Failed to delete buddy:', error); }
  };
      ...
      <div data-testid="settings-page" className="space-y-6 max-w-2xl">
        ...
        <div className="rounded-2xl p-6 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button data-testid="lang-en" onClick={() => setLang('en')} className="px-4 py-2 rounded-full text-sm font-medium transition-all" style={{ background: lang === 'en' ? '#6B9080' : '#F0EFEB', color: lang === 'en' ? '#FFF' : '#7A8B85' }}>English</button>
              <button data-testid="lang-da" onClick={() => setLang('da')} className="px-4 py-2 rounded-full text-sm font-medium transition-all" style={{ background: lang === 'da' ? '#6B9080' : '#F0EFEB', color: lang === 'da' ? '#FFF' : '#7A8B85' }}>Dansk</button>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-6 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-heading text-lg font-medium" style={{ color: '#2A3A35' }}>What I'm working on</h3>
          </div>
          <div className="grid grid-cols-3 gap-2 mb-3">
            {urgeTypes.map(ut => (
              <button key={ut.id} data-testid={`setting-urge-${ut.id}`} onClick={() => setSelectedUrgeType(ut.id)}
                className="p-3 rounded-xl text-xs font-medium text-center transition-all"
                style={{ background: selectedUrgeType === ut.id ? '#6B908015' : '#F9F8F6', border: selectedUrgeType === ut.id ? '2px solid #6B9080' : '1px solid #E8E6E1', color: selectedUrgeType === ut.id ? '#6B9080' : '#7A8B85' }}>
                {ut.label}
              </button>
            ))}
          </div>
          {selectedUrgeType === 'other' && (
            <Input data-testid="custom-urge-settings" value={customUrge} onChange={e => setCustomUrge(e.target.value)} placeholder="Describe..." className="rounded-xl mb-3" style={{ border: '1px solid #E8E6E1' }} />
          )}
          <Button data-testid="save-urge-type" onClick={saveUrgeType} className="rounded-full text-white font-medium text-sm" style={{ background: '#6B9080' }}>Save</Button>
        </div>
        ...
        <div className="rounded-2xl p-6 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <span className="text-xs px-2 py-0.5 rounded-full mt-1 inline-block" style={{ background: isPaid ? '#6B908022' : '#F0EFEB', color: isPaid ? '#6B9080' : '#A3B1AA' }}>{isPaid ? 'Pro' : 'Free'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-6 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Users className="w-5 h-5" style={{ color: '#6B9080' }} strokeWidth={1.5} />
              <h3 className="font-heading text-lg font-medium" style={{ color: '#2A3A35' }}>Accountability Buddy</h3>
            </div>
            {isPaid && (
              <Dialog open={buddyDialogOpen} onOpenChange={setBuddyDialogOpen}>
                <DialogTrigger asChild>
                  <Button data-testid="add-buddy-btn" variant="outline" className="rounded-full text-sm" style={{ border: '1px solid #E8E6E1', color: '#7A8B85' }}>Add Buddy</Button>
                </DialogTrigger>
                <DialogContent className="rounded-2xl" style={{ border: '1px solid #E8E6E1' }}>
                  <DialogHeader><DialogTitle className="font-heading text-xl" style={{ color: '#2A3A35' }}>Add a Buddy</DialogTitle></DialogHeader>
                  <div className="space-y-3 mt-4">
                    <p className="text-sm" style={{ color: '#7A8B85' }}>Add someone you trust. They can be notified if you're struggling.</p>
                    <Input data-testid="buddy-name" value={buddyName} onChange={e => setBuddyName(e.target.value)} placeholder="Name" className="rounded-xl" style={{ border: '1px solid #E8E6E1' }} />
                    <Input data-testid="buddy-email" value={buddyEmail} onChange={e => setBuddyEmail(e.target.value)} placeholder="Email (optional)" className="rounded-xl" style={{ border: '1px solid #E8E6E1' }} />
                    <Input data-testid="buddy-phone" value={buddyPhone} onChange={e => setBuddyPhone(e.target.value)} placeholder="Phone (optional)" className="rounded-xl" style={{ border: '1px solid #E8E6E1' }} />
                    <Input data-testid="buddy-relation" value={buddyRelation} onChange={e => setBuddyRelation(e.target.value)} placeholder="Relationship (friend, partner, sponsor...)" className="rounded-xl" style={{ border: '1px solid #E8E6E1' }} />
                    <Button data-testid="save-buddy" onClick={addBuddy} disabled={!buddyName.trim()} className="w-full rounded-full text-white font-medium h-11" style={{ background: '#6B9080' }}>Add Buddy</Button>
                  </div>
                </DialogContent>
              </Dialog>
            )}
          ...
          {isPaid ? (
            buddies.length > 0 ? (
              <div className="space-y-3">
                {buddies.map(b => (
                  <div key={b.buddy_id} className="flex items-center justify-between p-4 rounded-xl" style={{ background: '#F9F8F6' }}>
                    <div>
                      <p className="text-sm font-medium" style={{ color: '#2A3A35' }}>{b.name}</p>
                      <p className="text-xs" style={{ color: '#A3B1AA' }}>{b.relationship || 'Contact'} {b.email && `- ${b.email}`}</p>
                    </div>
                    <div className="flex gap-2">
                      <Button data-testid={`alert-buddy-${b.buddy_id}`} onClick={() => alertBuddy(b.buddy_id)} variant="outline" className="rounded-full text-xs px-3 h-8" style={{ border: '1px solid #E5989B44', color: '#E5989B' }}>
                        <AlertCircle className="w-3 h-3 mr-1" strokeWidth={1.5} /> Alert
                      </Button>
                      <button onClick={() => deleteBuddy(b.buddy_id)} className="p-1.5 rounded-lg" style={{ color: '#A3B1AA' }}>
              ...
              <p className="text-sm" style={{ color: '#A3B1AA' }}>No buddies added yet. Add someone you trust for accountability.</p>
            )
          ) : (
            <UpgradePrompt feature="Accountability Buddy">
              <div className="p-4 rounded-xl" style={{ background: '#F9F8F6' }}>
                <p className="text-sm" style={{ color: '#7A8B85' }}>Connect with someone who can support you when things get tough.</p>
              </div>
            </UpgradePrompt>
          )}
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <button data-testid="add-reminder-time" onClick={addTime} className="text-sm font-medium" style={{ color: '#6B9080' }}>{t('add_time')}</button>
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {DAYS_KEYS.map((day, i) => (
                    <button key={day} data-testid={`day-${day.toLowerCase()}`} onClick={() => toggleDay(day)} className="px-3 py-2 rounded-lg text-xs font-medium transition-all"
                      style={{ background: reminders.days.includes(day) ? '#6B9080' : '#F0EFEB', color: reminders.days.includes(day) ? '#FFF' : '#7A8B85' }}>{t(DAY_SHORT_KEYS[i])}</button>
                  ))}
                </div>
              </div>
