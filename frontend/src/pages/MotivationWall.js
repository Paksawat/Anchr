...
import { Heart, Plus, Trash2, Quote, Sparkles } from 'lucide-react';
      ...
      <div data-testid="motivation-page" className="space-y-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-2" style={{ color: '#7A8B85', fontFamily: 'Figtree, sans-serif' }}>
              Your words of strength, always here when you need them
            </p>
          </div>
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="rounded-2xl" style={{ border: '1px solid #E8E6E1' }}>
              <DialogHeader>
                <DialogTitle className="font-heading text-xl" style={{ color: '#2A3A35' }}>New Motivation</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 mt-4">
                <div>
                  <label className="text-sm mb-2 block" style={{ color: '#7A8B85' }}>Category</label>
                  <Select value={category} onValueChange={setCategory}>
                    <SelectTrigger data-testid="motivation-category-select" className="rounded-xl" style={{ border: '1px solid #E8E6E1' }}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
        ...
        {loading ? (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <span
                      ...
                      style={{ background: `${categoryColor(mot.category)}22`, color: categoryColor(mot.category) }}
                    >
                      {CATEGORY_LABELS[mot.category] || mot.category}
                    </span>
                    <button
                  ...
                  <p className="text-base leading-relaxed" style={{ color: '#2A3A35', fontFamily: 'Figtree, sans-serif' }}>
                    {mot.message}
                  </p>
          ...
          <div className="text-center py-16">
            <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center" style={{ background: '#F0EFEB' }}>
              <Sparkles className="w-8 h-8" style={{ color: '#A3B1AA' }} strokeWidth={1.5} />
            </div>
            <h3 className="font-heading text-xl" style={{ color: '#2A3A35' }}>No messages yet</h3>
            <p className="mt-2 text-sm" style={{ color: '#A3B1AA' }}>Write yourself a note of encouragement</p>
          </div>
        )}
      </div>
    </AppLayout>
