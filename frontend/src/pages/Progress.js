...
import { TrendingUp, Target, Clock, ArrowUp, ArrowDown } from 'lucide-react';
      ...
      <div data-testid="progress-page" className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="rounded-2xl p-5 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
            <span className="text-sm" style={{ color: '#A3B1AA' }}>{t('streak')}</span>
            <p data-testid="progress-streak" className="text-2xl font-heading font-light mt-1" style={{ color: '#2A3A35' }}>{stats?.streak_days || 0} {t('days')}</p>
          </div>
          <div className="rounded-2xl p-5 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
            <span className="text-sm" style={{ color: '#A3B1AA' }}>{t('resisted')}</span>
            <p className="text-2xl font-heading font-light mt-1" style={{ color: '#2A3A35' }}>{stats?.urges_resisted || 0}</p>
          </div>
          <div className="rounded-2xl p-5 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
            <span className="text-sm" style={{ color: '#A3B1AA' }}>{t('resist_rate')}</span>
            <p className="text-2xl font-heading font-light mt-1" style={{ color: '#2A3A35' }}>{stats?.resist_rate || 0}%</p>
          </div>
          <div className="rounded-2xl p-5 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
            <span className="text-sm" style={{ color: '#A3B1AA' }}>{t('best_streak')}</span>
            <p className="text-2xl font-heading font-light mt-1" style={{ color: '#2A3A35' }}>{stats?.best_streak || 0} {t('days')}</p>
          </div>
        </div>
          ...
          <div className="rounded-2xl p-6 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
            ...
            {triggerStats?.top_trigger && (
              <div className="mb-4 p-3 rounded-xl" style={{ background: '#A4C3B222' }}>
                <p className="text-sm font-medium" style={{ color: '#6B9080' }}><Target className="w-4 h-4 inline mr-1" strokeWidth={1.5} />{triggerStats.top_trigger} {t('is_top_trigger')}</p>
              </div>
            )}
              ...
              {triggerStats?.triggers?.slice(0, 5).map((tr) => {
                ...
                return (
                  <div key={tr.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-3 rounded-full overflow-hidden" style={{ background: '#F0EFEB' }}><div className="h-full rounded-full transition-all duration-500" style={{ width: `${(tr.count / maxCount) * 100}%`, background: '#6B9080' }} /></div>
                    <span className="text-xs w-6 text-right" style={{ color: '#A3B1AA' }}>{tr.count}</span>
                  </div>
                );
              })}
              {(!triggerStats?.triggers || triggerStats.triggers.length === 0) && <p className="text-sm" style={{ color: '#A3B1AA' }}>{t('no_trigger_data')}</p>}
            </div>
          </div>
          <div className="rounded-2xl p-6 shadow-sm" style={{ background: '#FFFFFF', border: '1px solid #E8E6E1' }}>
            ...
            {triggerStats?.peak_hour != null && (
              <div className="mb-4 p-3 rounded-xl" style={{ background: '#E2D4C844' }}>
                <p className="text-sm font-medium" style={{ color: '#7A8B85' }}><Clock className="w-4 h-4 inline mr-1" strokeWidth={1.5} />{t('most_urges_at')} {formatHour(triggerStats.peak_hour)}</p>
              </div>
            )}
              ...
              <ResponsiveContainer width="100%" height={150}>
                <BarChart data={triggerStats.hours}>
                  ...
                  <Tooltip content={({ active, payload }) => active && payload?.length ? (
                    <div className="rounded-lg p-2 shadow" style={{ background: '#FFF', border: '1px solid #E8E6E1' }}><p className="text-xs" style={{ color: '#2A3A35' }}>{formatHour(payload[0]?.payload?.hour)}: {payload[0]?.value}</p></div>
                  ) : null} />
                  <Bar dataKey="count" radius={[4, 4, 0, 0]}>{triggerStats.hours.map((entry) => <Cell key={`hour-${entry.hour}`} fill={entry.hour === triggerStats.peak_hour ? '#E5989B' : '#A4C3B2'} />)}</Bar>
                </BarChart>
              </ResponsiveContainer>
              ...
              {urges.slice(0, 10).map((u) => (
                <div key={u.urge_id} className="flex items-center justify-between p-3 rounded-xl" style={{ background: '#F9F8F6' }}>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: u.outcome === 'resisted' ? '#6B908033' : u.outcome === 'relapsed' ? '#E5989B33' : '#E2D4C844' }}>
                    ...
                    <div>
                      <p className="text-sm font-medium" style={{ color: '#2A3A35' }}>{u.trigger || t('no_trigger')} - {u.emotion || t('no_emotion')}</p>
                      <p className="text-xs" style={{ color: '#A3B1AA' }}>{new Date(u.created_at).toLocaleDateString()} at {new Date(u.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                    </div>
                  </div>
