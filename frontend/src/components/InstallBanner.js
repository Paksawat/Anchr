...
export default function InstallBanner() {
  ...
  return (
    ...
    >
      {/* iOS step-by-step sheet — slides up above the banner */}
      {isIOS && showIOSSteps && (
        <div
        ...
        >
          <p
            ...
            style={{ color: '#7A8B85' }}
          >
            To install Anchr on your iPhone or iPad:
          </p>
          <ol className="space-y-2">
            {[
              { icon: <Share className="w-4 h-4 shrink-0" style={{ color: '#6B9080' }} strokeWidth={1.5} />, text: 'Tap the Share button in Safari\'s toolbar' },
              { icon: <span className="w-4 h-4 shrink-0 text-center text-xs font-bold" style={{ color: '#6B9080' }}>+</span>, text: 'Scroll down and tap "Add to Home Screen"' },
              { icon: <span className="w-4 h-4 shrink-0 text-center text-xs font-bold" style={{ color: '#6B9080' }}>✓</span>, text: 'Tap "Add" to confirm' },
            ].map(({ icon, text }, i) => (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p
            ...
            style={{ color: '#A4C3B2' }}
          >
            {isIOS ? 'Add to your Home Screen' : 'Add to your home screen'}
          </p>
        </div>
        <button
